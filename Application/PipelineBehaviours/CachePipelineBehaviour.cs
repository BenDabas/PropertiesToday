...
namespace Application.PipelineBehaviours
{
        ...
        where TRequest : IRequest<TResponse>, ICacheable
    {
        ...
        public CachePipelineBehaviour(IDistributedCache cache, IOptions<CacheSettingsModel> cacheSettings)
        {
            _cache = cache;
        ...
        {
            // request Inherit from ICacheable.
            ...
            async Task<TResponse> GetResponseAndWriteToCacheAsync()
            {
                ...
                if(response != null)
                {
                    var slidingExperation = request.SlidingExpiration == null ?
                        TimeSpan.FromMinutes(_cacheSettings.SlidingExpiration)
                        : request.SlidingExpiration;

                    var cacheOptions = new DistributedCacheEntryOptions
                    {
                        SlidingExpiration = slidingExperation,
                    ...
                    var serializedData = Encoding.Default
                        .GetBytes(
                            JsonConvert
                            .SerializeObject(response, Formatting.Indented, new JsonSerializerSettings()
                            {
